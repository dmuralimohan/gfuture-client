...
const Cart = () => {
  ...
  const { items, updateQuantity, removeItem, subtotal, platformFee, platformFeeRate, extraFeeLabel, extraFeeAmount, total, totalItems } = useCart();
          ...
          >
            ...
            >
              <CardContent sx={ { p: 3 } }>
                ...
                { extraFeeLabel && extraFeeAmount > 0 && (
                  <Box sx={ { display: 'flex', justifyContent: 'space-between', mb: 1.5 } }>
                    <Typography variant="body2" color="text.secondary">
                      { extraFeeLabel }
                    </Typography>
                    <Typography variant="body2" fontWeight={ 600 }>₹{ extraFeeAmount.toFixed(2) }</Typography>
                  </Box>
                ) }
